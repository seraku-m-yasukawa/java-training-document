# Java研修スケジュール 1日目
講義内容の詳細は[別紙](1日目.html "1日目")に記載。

| タイムテーブル ||
| :--- | :--- |
| 09:30 - 11:30 | プログラミング、アルゴリズムについて |
| 11:30 - 12:30 | Javaについて |
| 12:30 - 13:00 | 初めてのプログラム |
| 14:00 - 14:45 | 構文、変数とフロー |
| 14:45 - 15:30 | 条件分岐 |
| 15:30 - 16:15 | くりかえし処理 |
| 16:15 - 18:00 | CodeWarsで基礎練習 |


---

## プログラミング、アルゴリズムについて
- [挨拶](1日目.html#挨拶)～[プログラミングで肝心な処理](1日目.html#プログラミングで肝心な処理)
プログラミングとは何か、なぜやるのかを説明。
アルゴリズムとは何か、なぜやるのかを説明。
...
### 課題
- [課題1](1日目.html#課題1)
研修生にそれぞれに、日本語でアルゴリズムとなるように何かのアルゴリズム（手順リスト的な物）を作ってもらい、  
それぞれのリストに書いてある通りのことをして、物ができるかを試して、アルゴリズムと考えること体験してもらう

**アルゴリズムのお題例**
- おにぎりを作る 
- サンドイッチを作る
- 国際センターエレベーターで地下1階から7階へ行く
- 蛍光灯を交換する
- セルフレジで買い物を支払う

## Javaについて
- [コーディング の前に](1日目.html#コーディング-の前に)～[javaの話](1日目.html#javaの話)
Javaの簡単な歴史、誕生とWORA(Write Once Run Anywhere)について説明  
JRE, JDK, JVMについて説明  
Javaのインストールと環境準備(OracleのopenJDKを使用)  
IDEのダウンロード
...
## 初めてのプログラム
- [初めてのJava作成](1日目.html#初めてのjava作成)～[Eclipseの起動](1日目.html#eclipseの起動)
メモ帳などのテキストエディタで初めてのJavaクラスを作成し、コマンドプロンプトでコンパイルして実行する。  
その後にeclipseをダウンロードして、同じ事をする。

## 構文、変数とフロー
- [本格的なjava](1日目.html#本格的なjava)
Javaの構文にて基礎的な部分を説明する。（Camel Case、文末の；等）

変数とは何か、どう使いJavaではどう定義するのかを簡単なプログラムで説明する  
クラスを実行したときに、メインメソッドが一番最初に呼ばれることなど簡単な処理フローを説明する
...
くりかえし処理について説明する。  
`while`, `for(;;)`,`for(:)`  
forEachはラムダ処理なのでこの段階では触れない

## CodeWarsでの基礎練習
- [コードウォーズ](1日目.html#コードウォーズ)
CodeWarsに登録してもらう  
CodeWarsの使い方を説明する  
8kyuの問題を1,2問一緒に説いて説明をする。

配列問題は飛ばして実施するように伝えておく
